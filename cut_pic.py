...
def get_cut_line_none_precut(img, rows, cols):
    ...
    # verify
    assert abs(col_lines[-1]-col_lines[-2]-xstep)/xstep < 0.2
    assert abs(row_lines[-1]-row_lines[-2]-ystep)/ystep < 0.2
...
if __name__ == '__main__':
    ...
    cut_images, (row_lines, col_lines) = grid_cut(img_data, rows, cols, False)